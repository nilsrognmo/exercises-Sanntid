...
func resourceManager(takeLow chan Resource, takeHigh chan Resource, giveBack chan Resource) {
	...
	res := Resource{}       //den delte ressursen

	for {
		select {
		case res = <-giveBack:
            //hvis ressusen blir returnert, ress oppdateres
		default:
			select {
			case takeHigh <- res:
                    //hvis høy pri motas, send resurs til tråden
				res = <-giveBack //venter til ressursen returnerer
			default:
                //ingen høy, skjekker lav
				select {
				case takeLow <- res:
                    //hvis lavpri mottas, send ressurs til tråd
					res = <-giveBack //venter til ressursen returnerer

                    //hvis høy motas mens lav er opptatt, prioriter høy
				case takeHigh <- res:
					res = <-giveBack //venter til ressursen returnerer
				}
			}
